<script lang="ts">
	...
	async function fetchEmployment() {
		const res = await fetch('https://sos2425-14.onrender.com/api/v1/proxy/employment-data');
		const json = await res.json();
	...
	function initChart() {
		...
		const projectCounts = communities.map(
			(c) => waterData.find((w) => w.autonomous_community === c)?.project_count || 0
		);
		const employmentRates = communities.map(
...
<main>
	...
	<canvas bind:this={canvasEl} style="max-width:80%; margin:20px auto; display:block; height:400px;"
	></canvas>
